...
pandemic_cum_split <- initial_split(pandemic_cum, prop = 0.80, strata = proplog)
...
Creating Recipe:
```{r}
pandemic_cum_rec <- recipe(proplog ~ ., data = pandemic_cum_train) %>%
...
qda_fit <- fit(qda_wkflow, pandemic_cum_train)
```
...
```{r}
predict(qda_fit, new_data = pandemic_cum_train, type = "prob")
```

We can view a confidence matrix and calculate accuracy on the **training data**:

```{r}
augment(qda_fit, new_data = pandemic_cum_train) %>%
  conf_mat(truth = proplog, estimate = .pred_class) 
```

```{r}
qda_acc <- augment(qda_fit, new_data = pandemic_cum_train) %>%
  accuracy(truth = proplog, estimate = .pred_class)
qda_acc
```
