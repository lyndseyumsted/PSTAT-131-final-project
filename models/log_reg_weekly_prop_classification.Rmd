---
title: "logistic regression on weekly proportions"
...
pandemic_weekly <- pandemic_weekly %>%
  select(-c("prop", "new_cases", "population", "ID_co", "proplog"))

pandemic_weekly_split <- initial_split(pandemic_weekly, prop = 0.80, strata = risk)
...
Creating Recipe:
```{r}
pandemic_weekly_rec <- recipe(risk ~ ., data = pandemic_weekly_train) %>%
...
qda_fit_weekly <- fit(qda_wkflow, pandemic_weekly_train)
```
...
```{r}
predict(qda_fit_weekly, new_data = pandemic_weekly_train, type = "prob")
```

We can view a confidence matrix and calculate accuracy on the **training data**:

```{r}
augment(qda_fit_weekly, new_data = pandemic_weekly_train) %>%
  conf_mat(truth = risk, estimate = .pred_class) 
```

```{r}
qda_acc <- augment(qda_fit_weekly, new_data = pandemic_weekly_train) %>%
  accuracy(truth = risk, estimate = .pred_class)
qda_acc
```
...
```{r}
predict(qda_fit_weekly, new_data = pandemic_weekly_test, type = "prob")
```

We can view the confusion matrix on the **testing** data:

```{r}
augment(qda_fit_weekly, new_data = pandemic_weekly_test) %>%
  conf_mat(truth = risk, estimate = .pred_class) 
```
...
ROC Curve:
```{r}
roc <- augment(qda_fit_weekly, pandemic_weekly_test)
...
```{r}
predict(nb_fit, new_data = pandemic_weekly_test, type = "prob")
```

We can view the confusion matrix on the **testing** data:

```{r}
augment(nb_fit, new_data = pandemic_weekly_test) %>%
  conf_mat(truth = risk, estimate = .pred_class) 
```
...
ROC Curve:
```{r}
roc <- augment(nb_fit, pandemic_weekly_test)
