...
```{r, echo=FALSE}
pandemic_cum %>% 
  dplyr::select("risk", "Age11_to_64_.") %>%
  mutate(Age11_to_64_. = cut(Age11_to_64_., 
                                  breaks = seq(min(Age11_to_64_.), 
                                  max(Age11_to_64_.), by = 1), 
                          include.lowest = TRUE)) %>%
  group_by(Age11_to_64_.) %>%
  na.omit(Age11_to_64_.) %>%
  ggplot(aes(Age11_to_64_.)) + 
  geom_bar(aes(fill = risk)) +
  scale_fill_manual(values = c("light blue", "pink", "light green")) +
  theme(axis.text.x = element_text(angle = 90))
```
